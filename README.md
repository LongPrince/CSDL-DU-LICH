...
| :--- | :--- |
| **Mô tả** | Chuyên viên quản lý nghiệp vụ hoặc doanh nghiệp thực hiện xem danh sách, tìm kiếm, lọc các nhà hàng, quán ăn, cửa hàng đặc sản, cơ sở mua sắm đạt chuẩn phục vụ khách du lịch. |
...
| **Điều kiện kết thúc** | Xem được danh sách thông tin cơ sở dịch vụ theo các tiêu chí đã chọn. |
| **Kịch bản chính** | 1. Chọn menu "Ăn uống & Mua sắm". Hệ thống hiển thị danh sách cơ sở.<br>2. Người dùng sử dụng các bộ lọc hoặc ô tìm kiếm để tra cứu thông tin cơ sở cần tìm.<br>3. Hệ thống trả về kết quả danh sách các cơ sở ăn uống/mua sắm phù hợp. |
| **Kịch bản phụ** | - *Không tìm thấy kết quả:* Hệ thống thông báo "Không tìm thấy dữ liệu" nếu từ khóa hoặc bộ lọc không khớp với bất kỳ cơ sở nào. |
| **Các yêu cầu, ràng buộc** | Danh sách cần được phân trang để đảm bảo tốc độ tải trang khi dữ liệu lớn. |
...
| Thêm cơ sở mới | Button | Nút mở giao diện thêm mới (nếu có quyền). |
...
| **Tên chức năng** | **Mô tả** | **Thành công** | **Thất bại** |
| :--- | :--- | :--- | :--- |
| **Lọc và Tìm kiếm** | Thực hiện tra cứu cơ sở theo tiêu chí. | Hiển thị danh sách phù hợp. | Không có dữ liệu, báo trống. |
...
| **Điều kiện kết thúc** | Xem được danh sách các điểm đến trên hệ thống. |
| **Kịch bản chính** | 1. Truy cập menu "Điểm đến".<br>2. Hệ thống hiển thị danh sách các điểm đến du lịch nổi bật trên địa bàn.<br>3. Người dùng nhập tên hoặc chọn danh mục để tra cứu điểm đến. |
| **Kịch bản phụ** | - *Lỗi tải dữ liệu:* Hệ thống thông báo lỗi kết nối máy chủ nếu không thể lấy danh sách điểm đến. |
| **Các yêu cầu, ràng buộc** | (Không có) |
...
| :--- | :--- |
| **Mô tả** | Giao diện hiển thị các điểm đến nổi bật của địa phương, phục vụ cho việc quản lý thông tin quảng bá. |
| **Màn hình kết nối** | (Không có) |

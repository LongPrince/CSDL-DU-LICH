...
| **Tên Use case** | **Quản lý danh sách và thông tin Cơ sở lưu trú** |
| :--- | :--- |
| **Mô tả** | Cung cấp các công cụ quản lý toàn diện thông tin các cơ sở lưu trú du lịch (Khách sạn, Homestay, Resort, Biệt thự...) trên địa bàn. Hỗ trợ cập nhật thông tin vị trí, hạng sao, tiện ích đi kèm và cấu hình chi tiết các loại phòng phục vụ khách du lịch. |
| **Tác nhân** | Quản trị viên hệ thống (Admin/QTV), Người dùng Doanh nghiệp lưu trú |
| **Độ phức tạp** | 🔲 Đơn giản  ☑️ Trung bình  🔲 Phức tạp |
| **Điều kiện bắt đầu** | - Đối với Admin: Truy cập phân hệ "Quản lý nội dung" &rarr; Chọn "Cơ sở lưu trú".<br>- Đối với Doanh nghiệp: Chọn mục "Thông tin cơ sở" tại trang quản trị dành cho chủ cơ sở lưu trú đã được duyệt hồ sơ. |
| **Điều kiện kết thúc** | Thông tin cơ sở lưu trú và danh sách phòng được cập nhật thành công, hiển thị chính xác trên Bản đồ số và Cổng thông tin du lịch. |

---

#### a. Danh sách cơ sở lưu trú

<p align="center"><i>Hình 3.16: Giao diện Danh sách Cơ sở lưu trú du lịch</i></p>

| **Màn hình** | **Danh sách Cơ sở lưu trú du lịch** |
| :--- | :--- |
| **Mô tả** | Màn hình danh sách tập trung giúp Admin kiểm soát trạng thái hoạt động, hạng sao và thông tin cơ bản của toàn bộ cơ sở lưu trú, hoặc giúp Doanh nghiệp xem danh sách các chi nhánh thuộc sở hữu. |
| **Màn hình kết nối** | Chi tiết và Cập nhật cơ sở lưu trú, Quản lý loại phòng |

**Nội dung màn hình:**

| **Item** | **Kiểu dữ liệu** | **Mô tả** |
| :--- | :--- | :--- |
| Ô tìm kiếm nhanh | Textfield – String | Tìm kiếm cơ sở lưu trú theo Tên cơ sở hoặc Số điện thoại. |
| Lọc loại hình lưu trú | Combobox | Lọc theo danh mục: Khách sạn, Homestay, Resort, Nhà nghỉ... (Mặc định: Tất cả). |
| Lọc theo địa bàn | Combobox | Lọc cơ sở lưu trú theo Quận/Huyện (VD: Phú Vang, Quảng Điền, TP. Huế...). |
| Lọc theo hạng sao | Combobox | Lọc theo tiêu chuẩn xếp hạng: Chưa xếp hạng, 1 Sao, 2 Sao... đến 5 Sao. |
| Nút Thêm Mới | Button | Kích hoạt luồng tạo mới một cơ sở lưu trú pháp nhân trên hệ thống. |
| Bảng cơ sở lưu trú | Table | Khung hiển thị danh sách các cơ sở dưới dạng bảng dữ liệu. |
| *STT* | Label – Number | Số thứ tự dòng. |
| *Tên cơ sở* | Label – String | Tên thương mại của cơ sở lưu trú (VD: Silk Path Grand Huế, Alba Wellness). |
| *Loại hình* | Label – String | Nhóm phân loại lưu trú tương ứng (VD: Khách sạn, Resort). |
| *Hạng sao* | Label – String | Số sao được công nhận (VD: 5 Sao, 3 Sao hoặc N/A). |
| *Địa bàn* | Label – String | Khu vực hành chính trực thuộc (VD: Phường Vĩnh Ninh, TP. Huế). |
| *Trạng thái* | Badge | Trạng thái hoạt động: Đang hoạt động (Xanh), Tạm ngưng (Đỏ). |
| *Thao tác* | Icon Buttons | Nhóm biểu tượng điều hướng: Xem/Sửa thông tin, Quản lý phòng, Xóa cơ sở. |
| Thống kê tổng số | Card – Number | Khối hiển thị tổng số cơ sở lưu trú hiện có trên hệ thống (VD: 145). |
| Điều hướng trang | Pagination | Thanh phân trang dữ liệu (Trước, 1, 2, 3..., Sau). |

**Các chức năng màn hình:**

| **Tên chức năng** | **Mô tả** | **Thành công** | **Thất bại** |
| :--- | :--- | :--- | :--- |
| **Tìm kiếm & Bộ lọc** | Thực hiện quét dữ liệu dựa trên sự kết hợp giữa từ khóa tìm kiếm và các tiêu chí phân loại. | Danh sách tự động tải lại và hiển thị các cơ sở thỏa mãn điều kiện lọc. | Không tìm thấy cơ sở nào thỏa mãn, bảng hiển thị trạng thái trống. |
